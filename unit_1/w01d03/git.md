...
1. `mkdir ~/dev` 
1. Go to a suitable location in the terminal (let's do `~/dev/`) and type `git clone ` and paste the ssh url (e.g.`git clone https://git.generalassemb.ly/arthurbernierjr/everythingisawesome.git`)
...
1. Add original remote repo with `git remote add upstream https://git.generalassemb.ly/arthurbernierjr/everythingisawesome.git`
